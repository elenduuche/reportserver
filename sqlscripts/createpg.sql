...
(
  ...
  narration character varying(150) DEFAULT NULL
);

ALTER table payments owner to :user;
...
DROP INDEX IF EXISTS payments_trxnref_idx;
CREATE INDEX ON Payments
(trxnref);

DROP INDEX IF EXISTS payments_senderid_idx;
CREATE INDEX ON Payments
(senderid);

DROP INDEX IF EXISTS payments_createdon_idx;
CREATE INDEX ON Payments
(createdon);
